...
- Disabled two-finger twist face turns on coarse mobile touch devices; mobile users now rotate faces with the toolbar buttons. Desktop trackpad twist remains enabled. Verified mobile gesture no-op and toolbar turn screenshot under output/web-game/mobile-no-twist/.
- Fixed toolbar button turn sign by mapping the clockwise/counter-clockwise icons to the opposite internal layer direction used by the inside-out view. Verified both mobile toolbar buttons under output/web-game/mobile-no-twist/*-icon-fixed.png.
